...
struct GravitationalBody
{
    ...
    bool isBounce = false; // Body only bounces, no accretion-type behavior. False makes it follow the standard thresholds. true makes it always bounce.
    // Attribute enablement flags
    bool isStatic = false;      // makes the particle not follow any forces, can only have a fixed initial velocity. Basically as if it has infinite mass.
    ...
    bool isMacroGhost = false;  // makes the body a ghost to macro bodies, but does not ghost for micro particles.

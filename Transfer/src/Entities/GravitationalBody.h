...
struct GravitationalBody {
    ...
    Vector2D previousPosition = {0.0,
                                 0.0}; // Previous frame position vector (used
    ...
    bool isMacro =
        false; // Is body with macro-particle and macro-macro gravitational
    ...
    bool isBounce =
        false; // Body only bounces, no accretion-type behavior. False makes it
               // follow the standard thresholds. true makes it always bounce.
    // Attribute enablement flags
    bool isStatic =
        false; // makes the particle not follow any forces, can only have a

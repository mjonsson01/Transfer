...
RadiusSlider::RadiusSlider() : Slider() {
    ...
    trackRect = SDL_FRect{1 * SCREEN_WIDTH / 3, 2 * SCREEN_HEIGHT / 3, 300, 12};
    knobRect = SDL_FRect{0, 0, 20, 30}; // will set x,y below
    // Dead zone
    float deadzonePaddingX = 30.0f;
    float deadzonePaddingY = 60.0f;
    hotZoneRect = SDL_FRect{trackRect.x - deadzonePaddingX / 2.0f,
    ...
    knobRect.x =
        trackRect.x +
        (sliderValue - minValue) / (maxValue - minValue) * trackRect.w -
        knobRect.w / 2.0f;
    knobRect.y = trackRect.y - (knobRect.h - trackRect.h) / 2.0f;

    setVisibility(true);
    setPosition(trackRect.x, trackRect.y);
    UIElementTypeIdentifier = UIElementType::RADIUS_SLIDER_INDEX;
}

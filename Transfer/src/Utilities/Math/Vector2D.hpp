...
struct Vector2D
{
    ...
    Vector2D operator*(double scalar) const { return {xVal * scalar, yVal * scalar}; }
    Vector2D operator/(double scalar) const { return {xVal / scalar, yVal / scalar}; }
    ...
    double magnitude() const { return sqrt(xVal * xVal + yVal * yVal); }
    double square_magnitude() const { return (xVal * xVal + yVal * yVal); }
    double dot(const Vector2D& other) { return xVal * other.xVal + yVal * other.yVal; }
...
static inline Vector2D lerp(const Vector2D& a, const Vector2D& b, double t) { return a + (b - a) * t; }

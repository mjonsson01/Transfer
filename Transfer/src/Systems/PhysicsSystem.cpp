...
//     return {distance,     unit_normal_vector, relative_velocity_vector,
//             normal_speed, abs_normal_speed,   shouldCollide,
//             shouldBlowUp};
// }
...
//     if (body1.radius == 0 ||
//         body2.radius ==
//             0) // this one might be okay because its cast to int? not sure
//             here
//         return;
...
//             body.velocity += current_acceleration * (PHYSICS_TIME_STEP
//             / 2.0);
//         }
...
//             {
//                 if (pair.heavy->isBounce) // even if going fast, should
//                 bounce
...
//                     {
//                         // std::cout << "Collision Branch Hit: 6" <<
//                         std::endl; substituteWithParticles(*pair.light,
//                         gameState); continue;
//                     }
//                     else
//                     {
//                         // std::cout << "Collision Branch Hit: 7" <<
//                         std::endl; handleElasticCollisions(*pair.light,
//                         *pair.heavy); continue;
//                     }
//                 }
...
//                     {
//                         // std::cout << "Collision Branch Hit: 8" <<
//                         std::endl; substituteWithParticles(*pair.heavy,
//                         gameState); continue;
//                     }
//                     else
//                     {
//                         // std::cout << "Collision Branch Hit: 9" <<
//                         std::endl; handleElasticCollisions(*pair.light,
//                         *pair.heavy); continue;
//                     }
//                 }
...
//                         // std::cout << "Collision Branch Hit: 10" <<
//                         std::endl;
...
//                     {
//                         // std::cout << "Collision Branch Hit: 11" <<
//                         std::endl; substituteWithParticles(*pair.light,
//                         gameState); continue;
//                     }
//                 }
//             }
//             // Now we know not going fast enough to blow up, we can attempt
//             to
//             // accrete
//             else
//             {
...
//                 getCollisionInfo(macro_body, particle);
//             if (collision_info.shouldCollide)
//             {
...
//                     {
//                         // std::cout << "Collision Branch Hit: 19" <<
//                         std::endl; handleAccretion(particle, macro_body);
...
//         normal_vector * (v_smaller_n_new - v_smaller_n) *
//         ELASTIC_LOSS_FACTOR;
...
//     // proxy?Add later?
//     double distance = (macroBody1.position -
//     macroBody2.position).magnitude(); int proxyBody1LifeTime =
//     randomDouble(0.5, 1.5) * TARGET_FPS * distance /
...
//     macros.insert(macros.end(), pending_macros.begin(),
//     pending_macros.end());
// }
...
//     // Add to radius
//     body.radius =
//         body.radius * pow((body.mass + particle.mass) / body.mass, 1.0
//         / 3.0);
...
// void PhysicsSystem::createParticle(GameState& gameState, InputState&
// inputState)
// {
//     GravitationalBody body;
//     body.mass = inputState.selectedMass;
...
//     {
//         totalEnergy +=
//             bodies[i].mass * bodies[i].velocity.square_magnitude() / 2.0;
//     }
...
//         {
//             double epsilon = (bodies[i].radius + bodies[j].radius) /
//                              2.0; // Simple average radius
...
//             totalEnergy -= GRAVITATIONAL_CONSTANT * bodies[i].mass *
//                            bodies[j].mass / denominator;
//         }
//     }
...
//                 pixelPositions.push_back(center +
...
//     // 4. CLEANUP
//     originalBody.isMarkedForDeletion = true; // Destroy the original macro
//     body
// }

// void PhysicsSystem::populateCollisionProxyFromMacroBody(
//     GravitationalBody& originalMacroBody, GravitationalBody& proxyBody)
// {
    ...
    GameState& gameState)
{
    ...
    // int proxyBody1LifeTime = randomDouble(0.9, 1.5) * TARGET_FPS * distance /
    //                          macroBody1.velocity.magnitude();

    // int proxyBody2LifeTime = randomDouble(0.9, 1.5) * TARGET_FPS * distance /
    //                          macroBody2.velocity.magnitude();

...
// void PhysicsSystem::substituteWithParticles(GravitationalBody& originalBody, GameState& gameState, double
// densityFactor)
// {
...
void PhysicsSystem::substituteWithParticles(GravitationalBody& originalBody, GameState& gameState)
{
    ...
    uint32_t num_particles =
        (uint32_t)round(PI * R * R / PARTICLE_DENSITY_DIVISOR); // maybe change to global density factor ?
    num_particles = std::max(1U, num_particles);                // make sure there is at least 1 particle.
    ...
    for (const auto& pos : pixelPositions)
    {
        ...
        p.mass = particleMass;
        // p.radius = 1.0;
        p.radius = OVERLAP_MARGIN * sqrt(PARTICLE_DENSITY_DIVISOR / PI);
        p.position = pos;
        p.previousPosition = pos; // No offset here
        p.isFragment = true;
        p.isAccretable = true;
                                                        ...
                                                        GravitationalBody& proxyBody)
{
    ...
    proxyBody.isMacroGhost = true;
    proxyBody.isMacro = true;
    // proxyBody.macroIdentifier = originalMacroBody.macroIdentifier; // Need to fix
    // proxyBody.lifetime = TARGET_FPS; // ~1 second for now, still need
    // to fix.

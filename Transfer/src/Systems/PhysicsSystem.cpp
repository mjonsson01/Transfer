...
void PhysicsSystem::integrateForwardsPhase2(GameState &gameState)
{
    ...
    for (auto &body : macro_bodies)
    {
        ...
        // Calculate new acceleration (a_t + dt) using the newly calculated netForce
        Vector2D new_acceleration = body.isStatic
                                        ? Vector2D(0.0, 0.0)
    ...
    for (size_t i = 0; i < num_macro_bodies; ++i)
    {
        ...
        for (size_t j = i + 1; j < num_macro_bodies; ++j)
        {
            ...
            else
            {
                ...
                // as long as the heavier/lighter is within 40%, blow em both to shit. will tweak as needed and place in engine constants when im happy with it
                // note that ratio alwasys >= 1.0 since I force the heavier to be in numerator
                if (pair.ratio <= 1.4)
                {
...
void PhysicsSystem::handleElasticCollisions(GravitationalBody &smallerBody, GravitationalBody &largerBody)
{
    ...
    // {
    //     GravitationalBody &bounceBody = smallerBody.isBounce ? smallerBody : largerBody;
    ...
    // Standalone Soln2
    // if (!smallerBody.isBounce && !largerBody.isBounce && (smallerBody.mass == 0 || largerBody.mass == 0))
    //     return;
...
{
    // 1. Setup constants and shared center
    const Vector2D center_of_explosion = (body1.position + body2.position) / 2.0;
    const double combined_radius = body1.radius + body2.radius;
    const double falloff_radius = 0.5 * combined_radius; // make smaller

    // Calculate a shared momentum-conserving velocity for the "crush zone"
    Vector2D blended_vel = (body1.velocity * body1.mass + body2.velocity * body2.mass) / (body1.mass + body2.mass);
    ...
    proxy.lifetime = TARGET_FPS; // sloppy solution for now but probably fine. NEED TO FIX
    double avgSpeed = (body1.velocity.magnitude() + body2.velocity.magnitude()) * 0.5;
    double avgMass = (body1.mass + body2.mass) / 2;
    double sizeFactor = (body1.radius + body2.radius) * 0.5;
    // lifetime in frames
    // proxy.lifetime = static_cast<uint32_t>(TARGET_FPS * (5 * sizeFactor / (avgSpeed + 1e-5)));
    // proxy.lifetime = std::min(static_cast<int>(proxy.lifetime), TARGET_FPS * 3); // max 3 seconds
    proxy.lifetime = TARGET_FPS * (1 + 5 * avgMass / MAX_MASS);
    // Note: You'll need to add a 'lifeSpan' or similar to your struct
        ...
        for (int i = -R_int; i <= R_int; ++i)
        {
            for (int j = -R_int; j <= R_int; ++j)
            {
                if ((i * i) + (j * j) <= (source.radius * source.radius))
                {
                    ...
                    // // Blends between the original body velocity and the collision momentum
                    // Vector2D base_vel = lerp(source.velocity, blended_vel, falloff);

                    // double factor = source.velocity.magnitude();
                    // constexpr double EXPLOSION_STRENGTH = 0.5; // 0.0 = no explosion, 1.0 = current strength
                    // double blast = factor * falloff * EXPLOSION_STRENGTH;
                    // double shear = factor * falloff * 1.5 * EXPLOSION_STRENGTH;
                    // double jitter = randomDouble(-1.0, 1.0) * falloff * EXPLOSION_STRENGTH;
                    // constexpr double RADIAL_SCALE = 0.6;
                    ...
                    // p.position = pos;
                    // p.isFragment = true;
                    // p.velocity = base_vel + n * (blast * RADIAL_SCALE + jitter) + t * (shear * TANGENTIAL_SCALE * randomDouble(-1.0, 1.0));
                    ...
                    double factor = source.velocity.magnitude();
                    double blast = factor * falloff * 0.75;
                    double shear = factor * falloff * 1.5; // Slight reduction in shear for stability
                    ...
                    p.position = pos;
                    p.isFragment = true;
                    p.velocity = base_vel + n * (blast + jitter) + t * (shear * randomDouble(-1.0, 1.0));
                    p.previousPosition = p.position - p.velocity * PHYSICS_TIME_STEP;
    ...
    auto &particles = gameState.getParticlesMutable();
    particles.insert(particles.end(), pending_particles.begin(), pending_particles.end());
